...
public class CalcControllerTest {
  ...
  @Test // Test 1
  void testObservableAndObserver()
      throws InvalidExpressionException, EmptyStackException, BadTypeException {
    /*
     * This test ensures that the calculate method in the CalcController class works as expected
     * when we set it to observe an observable like the GuiViewController.
     * 
     * To make this test work, I had to first create a CalcController class. This class has a model
     * attribute which references an instance of a model to perform calculations. The class has a
     * calculate() method which calls the model's evaluate() method which is different depending on
  ...
  void testChangingModelType() {
    /*
     * This test ensures that we can set the expression type through the view interface so that the
     * model's calculator state will update when the user presses the radio buttons.
     * 
